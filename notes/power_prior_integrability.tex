...
For $ 0 < x < 1$, we have
\[ (x-1)\log(x) \leq \log(x)\log(x) \leq \log(x) - \log(x)/x. \]
If we again make $X = L(D_0 \mid \theta)$ and take expectations with respect to the unormalised density $f_{a_0}(D_0; \theta)$, we have that $E[\log^2(X)] \leq E[\log(X)] - E[\log(X)/X]$.
Since $E\left[\log(X)\right]$ is simply $c^\prime(a_0)$ and $E[\log(X)/X]$ is bounded above by $c(a_0 + 1)$, we conclude $c^{\prime\prime}(a_0) < \infty$, completing the proof.
\end{proof}
