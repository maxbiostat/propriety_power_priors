...
\end{align}
when the integrals exist. %% LM: to see why, recall that L(D|t) bounds log L(D|t) above.
...
\begin{equation}
 c^\prime(a_0) = \log(h(D_0)) + \int_{\boldsymbol\Theta} f_{a_0}(D_0; \theta) \left[  \boldsymbol\eta(\theta) \cdot \boldsymbol T(D_0) \right] d\theta -  \int_{\boldsymbol\Theta} f_{a_0}(D_0; \theta)  A(\theta) d\theta,
\end{equation}
where $f_{a_0}(D_0; \theta) := \left[ h(D_0)\exp\left( \boldsymbol\eta(\theta) \cdot \boldsymbol T(D_0) - A(\theta) \right) \right]^{a_0}\pi(\theta)$.

Now suppose $\pi(\theta)$ is conjugate to $L(D_0 \mid \theta)$~\citep{Diaconis1979}:
\begin{equation}
...
A caveat of this grid approach is that the maximum end point needs to chosen in advance, effectively bounding the space of $a_0$ considered. 
...
In fact, prior information can be used to set the maximum: let $p$ be a fixed probability and then set the maximum grid value $M$ such that
\[ \int_0^M \pi(a) da = p. \]
...
In order to evaluate the effect of a discretisation of $g_{\boldsymbol\xi}$ on inferences, we evaluated the effect of varying the prediction grid size $K$.
...
We employed  bridge sampling to compute marginal likelihoods, i.e., $c(a_0)$, implemented in the R package~\textbf{bridgesampling}~\citep{Gronau2017}.
