...
compiled.model.prior <- stan_model("stan/simple_Bernoulli_posterior_unnormalised_fixed_a0.stan") 
...
adaptive.time <- system.time(
  adaptive.ca0.estimates <- build_grid_derivOnly(eps = epsilon,
                                       M = maxA, J = J, v1 = 10, v2 = 10, stan.list = bb.data, pars = "theta")
)
write.csv(adaptive.ca0.estimates$result,
          file = paste("../data/constant_data/Bernoulli_posterior_logCA0_scenario_", scenario,
...
write.csv(summaries.adaptive.dt,
          file = paste("../data/sensitivity_data/posteriorSensitivity_simpleBernoulli_adaptive_scenario_", scenario,
          "_J=", J, ".csv", sep = ""))