...
constant_data <- read.csv("../data/RegressionNIG_logCA0.csv")
library(mgcv)
...
names(unnorm.pars) <- c(paste("beta_", 1:4, sep = ""), "sigma_sq")
...
app.norm.pars.list <- lapply(all.approximates, function(x) {
  ...
  names(pars) <- c(paste("beta_", 1:4, sep = ""), "sigma_sq")
  return(pars)
})
...
names(norm.pars) <- c(paste("beta_", 1:4, sep = ""), "sigma_sq")
